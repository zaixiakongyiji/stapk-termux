...
1. **初次启动**：安装并打开 App，如果是第一次运行，后台会自动解压基础环境和组件。您可以点击 **"View Log"** 观察日志，确认看到类似 Payload Ready 的提示即代表解压完成。
2. **正常使用**：点击 **"Start Server"** 启动服务。稍等片刻后，您可以在下方内置的 WebView 直接浏览酒馆界面，或者点击 **"Open Browser"**，会自动在您手机自带的原生浏览器中打开。使用原生浏览器时，通知栏会显示一个驻留通知为您“保活”，防止聊天时服务被系统杀掉。
3. **数据备份**：点击 **"Backup Data"** 按钮，会弹出一个系统的文件保存窗口。找一个您记得住的文件夹，点击保存，您的聊天数据就会打包成 `.zip` 永久存放在您的手机里。
